<template>
  <alert :message="createGatewayStatus.message" :modalActive="modalActive" :isError="createGatewayStatus.isError"
            ...
            <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form">
              <form @submit="handleSubmit($event, onSubmit)" class="form-wrapper">
                ...
                <div class="flex justify-between gap-10">
                  ...
                  <BasicButton type="submit" class="primary" :label="registerLabel" :loading="createGatewayLoading" />
                </div>
              </form>
            </VeeForm>
          </div>
...
const { createGatewayStatus, createGatewayLoading } = storeToRefs(useGatewaysStore())
...
const onSubmit = async (values, { resetForm }) => {
  ...
  if (regButtonClick.value == 2) {
    ...
    if (createGatewayStatus.value.isError) {
