...
async function refreshToken() {
  try {
    const response = await axios.get(`${import.meta.env.VITE_APP_API_URL}auth/refresh`, {
      headers: {
    ...
    localStorage.clear('auth.role')
    router.push({ name: 'login' })
    return false
  }
}

export function createGatewayHealthEventSource(serialNumber) {
  let url = `${import.meta.env.VITE_APP_API_URL}telemetry/gateway-health/${serialNumber}`
